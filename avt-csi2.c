	...
	struct v4l2_mbus_framefmt *fmt)
{
	...
	avt_info(get_sd(camera), "fmt->code 0x%04X -> 0x%04X. rev_x %d rev_y %d", 
		old_code, fmt->code, 
...
static const char* get_media_bus_format_name(u32 format_id)
{
    switch (format_id) {
        // RGB formats
        ...
        // Metadata formats
        case MEDIA_BUS_FMT_METADATA_FIXED:
				 ...
				 const struct avt_binning_info **new_binning)
{
	...
	avt_dbg(get_sd(camera), "Incoming fmt->code    0x%04x %s", fmt->code, 
		get_media_bus_format_name(fmt->code));

	transform_mbus_code(sd, fmt);

	avt_dbg(get_sd(camera), "Transformed fmt->code 0x%04x %s", fmt->code, 
		get_media_bus_format_name(fmt->code));
